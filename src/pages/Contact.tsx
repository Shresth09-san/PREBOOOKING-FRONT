...
const Contact = () => {
  ...
  const handleChange = (e) => {
    ...
    setErrorMessage(""); // Clear any errors when the user types
  };

  const validateForm = () => {
    if (!formState.name.trim() || !formState.email.trim() || !formState.message.trim() || !formState.subject.trim()) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${backendUrl}/send-email`, formState);
      ...
      console.error("Error sending message:", error);
      const errorMsg = error.response?.data?.error || error.message || "Failed to send message. Please check your network and try again.";
        ...
        <div className="container-custom px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12 mt-16 sm:mt-20 md:mt-24">
            ...
            <div className="lg:col-span-3 bg-white rounded-2xl shadow-lg p-6 sm:p-8 md:p-10 border border-gray-100" style={{ textShadow: "2px 2px 4px rgba(0, 0, 0, 0.2)" }}>
              ...
              {successMessage && <div className="bg-green-50 text-green-700 p-4 rounded-lg mb-6">{successMessage}</div>}
              ...
              <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                ...
                <button
                  type="submit"
                  className="w-full bg-doit-600 hover:bg-doit-700 text-white py-2.5 px-6 rounded-lg flex items-center justify-center transition-colors"
                  disabled={isSubmitting}
                >
                  {isSubmitting ? (
                    <span className="flex items-center">
