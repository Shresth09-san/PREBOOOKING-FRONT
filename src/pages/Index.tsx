...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Animation for hero content with different timing for mobile vs desktop
      ...
      mm.add("(min-width: 768px)", () => {
        gsap.fromTo(
          heroContentRef.current,
          { y: 40, opacity: 0 },
          { y: 0, opacity: 1, duration: 1.5, delay: 0.7, ease: "power3.out" }
        );
      });
    });
    ...
    const handleResize = () => {
      // Refresh animations or adjust parameters if needed
      gsap.set(bgImageRef.current, { clearProps: "all" });
      gsap.set(heroContentRef.current, { clearProps: "all" });

      // Re-run animations if needed on major size changes
      ctx.revert();
      ctx.add(() => {
        gsap.fromTo(
          bgImageRef.current,
    ...
    <div className="min-h-screen">
      ...
      <section className="relative w-full h-screen flex items-center justify-center overflow-hidden px-4 sm:px-8">
        ...
        >
          ...
          {/* Main Heading */}
          <h1
            className="text-white text-2xl md:text-5xl lg:text-6xl xl:text-6xl font-bold mt-7 leading-tight drop-shadow-lg"
          ...
          {/* Description */}
          <p
            className="text-gray-200 text-base sm:text-lg md:text-xl xl:text-2xl mt-4 max-w-xl sm:max-w-2xl mx-auto leading-relaxed"
            style={{ textShadow: "2px 2px 4px rgba(0, 0, 0, 0.8)" }}
          >
          ...
          {/* Buttons */}
          <div className="mt-20 flex flex-wrap justify-center gap-6 w-full sm:w-auto">
            <Link to="/signup">
        ...
        <div className="container-custom">
          <div className="text-center max-w-2xl mx-auto mb-8 sm:mb-16 animate-fade-in">
            <h2
              ...
              style={{ textShadow: "2px 2px 4px rgba(128, 128, 128, 0.6)" }}
            >
              Our Services
            </h2>
            <p
              className="text-foreground/70 text-base sm:text-lg"
              style={{ textShadow: "2px 2px 4px rgba(128, 128, 128, 0.2)" }}
            >
      ...
      <section className="py-12 sm:py-16 md:py-20 px-4 md:px-8 bg-gradient-to-r from-doit-400 to-orange-500 text-white">
        <div className="container-custom text-center">
          <div className="max-w-2xl mx-auto md:px-6">
            <h2
              className="text-2xl sm:text-3xl md:text-3.5xl lg:text-4xl font-bold mb-4 md:mb-5 lg:mb-6 animate-fade-in"
              style={{ textShadow: "2px 2px 4px rgba(0, 0, 0, 0.6)" }}
            >
              Ready to Get Things Done?
            </h2>
            <p
              className="text-white/90 text-base sm:text-lg md:text-xl mb-6 md:mb-7 lg:mb-8 animate-fade-in"
